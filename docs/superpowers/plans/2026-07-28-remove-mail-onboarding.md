...
**Interfaces:**
- Consumes: 仓库根路径和 Mail 前端清单。
- Produces: `declares no retired mail onboarding module or dependency` 架构约束。
...
```powershell
pnpm --filter @zero/server exec vitest run src/mail-architecture.test.ts -t "declares no retired mail onboarding module or dependency"
```
