...
完全一致。迁移前后的差异应当仅包括 Schema 限定名、表名以及由此派生的数据库
对象名称。
...
4. 只重命名仍带 `mail0_` 的索引、主键、唯一约束和外键对象，不改变它们的定义；
...
3. 应用 Schema 化迁移；
4. 断言数据主键与行数不变；
5. 对比迁移前后的字段、类型、空值、默认值、约束和索引定义，忽略 Schema、
   表名及对象名称后必须完全相同；
6. 断言跨 Schema 外键可以阻止非法引用；
7. 断言 `public` 中没有 `mail0_*` 业务表。
