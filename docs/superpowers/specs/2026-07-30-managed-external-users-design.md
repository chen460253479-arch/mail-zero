...
5. 普通用户密码登录和 CRM Launch 使用相同的受限邮件界面。
6. 普通用户只能访问自己名下的全部邮箱。
7. 超管可以查看和管理实例内全部邮箱。
...
| 模式             | 认证方式                               | 邮箱范围               | 设置和连接管理 |
| ---------------- | -------------------------------------- | ---------------------- | -------------- |
| 超管完全模式     | 管理员邮箱和密码                       | 实例内全部邮箱         | 允许           |
| 普通用户受限模式 | `externalUserId` 和密码，或 CRM Launch | 当前用户拥有的全部邮箱 | 禁止           |

匿名请求不属于邮件使用模式，只能进入登录流程。

普通用户密码登录和 CRM Launch 的认证来源不同，但权限、邮箱范围、导航和邮件界面完全相同。
服务端可以保留认证来源信息，用于会话处理和审计，不得用它扩大普通用户权限。
...
现有只在 External 模式渲染的 `ExternalAccountSwitcher` 改为通用账号切换组件。
...
6. 两种普通用户登录方式显示相同受限界面和相同邮箱集合。
7. 普通用户不能访问其他用户邮箱、设置或连接管理。
8. 超管可以查看、切换和管理实例内全部邮箱。
9. 同一个 Nango Connection 不能属于两个普通用户。
10. Webhook 仍然只推送 `eventId` 和 `messageId`。
