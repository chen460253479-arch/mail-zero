...
export function normalizeKeyword(keyword: string): Keyword {
  if (keyword.length === 0 || keyword.length > 255 || invalidKeywordCharacter.test(keyword)) {
    throw new MailCoreError('INVALID_KEYWORD');
  }

  const normalizedStandardKeyword = keyword.toLowerCase();

  return standardKeywordSet.has(normalizedStandardKeyword) ? normalizedStandardKeyword : keyword;
}
