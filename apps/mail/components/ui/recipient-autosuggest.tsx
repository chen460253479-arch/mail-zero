...
interface RecipientAutosuggestProps<TFieldValues extends FieldValues> {
...
export function RecipientAutosuggest<TFieldValues extends FieldValues>({
  ...
  const {
    field: { value, onChange: onRecipientsChange },
  } = useController({
    control,
    name,
  });
  const recipients = useMemo(
    () =>
      Array.isArray(value)
        ? value.filter((recipient: unknown): recipient is string => typeof recipient === 'string')
        : [],
    [value],
  );
  ...
  const [, threads] = useThreads({ enabled: debouncedQuery.trim().length > 0 && !isComposing });
  const allSuggestions = useMemo<RecipientSuggestion[]>(() => {
    ...
    return Array.from(
      new Map(
        threads
          .map((thread) => thread.sender)
          .filter(
            (sender) =>
              sender.email &&
              `${sender.name ?? ''} ${sender.email}`.toLowerCase().includes(normalizedQuery),
          )
          .map((sender) => [
