...
export const emailSubmission = createMailTable(
  'submission',
  {
    ...
    rawSizeBytes: bigint('raw_size_bytes', { mode: 'bigint' }).notNull(),
  ...
  (t) => [
    ...
    check('email_submission_raw_size_nonnegative_check', sql`${t.rawSizeBytes} >= 0`),
    ...
    foreignKey({
      name: 'email_submission_raw_blob_account_fk',
    ...
    index('email_submission_raw_blob_account_idx').on(t.rawBlobId, t.mailAccountId),
