...
export type AdminProvisioningDependencies = {
  db: DB;
...
export const provisionAdmin = async (
  ...
  const workspace = dependencies.userWorkspace.forUser(result.userId);
...
export const createConfiguredAdminProvisioner = (
  environment: ZeroEnv,
  dependencies: AdminProvisioningDependencies,
) => {
  ...
  return () => {
    configuredProvisioning ??= (async () => {
      const credentials = parseAdminProvisioningConfig(environment);
      if (!credentials) return null;
      return provisionAdmin(credentials, dependencies);
    })();
